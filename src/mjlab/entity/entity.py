...
class Entity:
  ...
  def _identify_joints(self) -> None:
    ...
    # Ball joints are multi-DOF passive constraint joints. Exclude them from the
    # observable joint state: their quaternion qpos is incompatible with mjlab's
    # per-joint-name indexing. They still participate fully in physics.
    self._non_free_joints = tuple(
      j for j in self._non_free_joints if j.type != mujoco.mjtJoint.mjJNT_BALL
    )
  ...
  def initialize(
    ...
    if self.is_articulated:
      if self.cfg.init_state.joint_pos is None:
        # Use keyframe joint positions, selecting only observable (non-ball) joints
        # via joint_q_adr so the shape matches data.joint_pos.
        key_qpos = mj_model.key("init_state").qpos
        default_joint_pos = torch.tensor(key_qpos, dtype=torch.float, device=device)[
          indexing.joint_q_adr
