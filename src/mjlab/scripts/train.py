...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  ...
  # Save a reproducibility snapshot (rank 0 only to avoid races):
  #   git/commit.txt       — exact commit hash
  #   git/clean.patch      — plain `git diff` replayable with `git apply`
  #   src/<path>           — full copy of every modified source file
  if rank == 0:
    import shutil
    import subprocess
    ...
    try:
      ...
      (git_dir / "commit.txt").write_text(commit + "\n")
      diff = subprocess.check_output(["git", "diff"], text=True)
      (git_dir / "clean.patch").write_text(diff)
      # Copy each modified file so the exact source is always available.
